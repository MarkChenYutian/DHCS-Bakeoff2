...
const dbg_curr_dist        = document.getElementById("dbg-curr-dist");
const dbg_orig_dist        = document.getElementById("dbg-orig-dist");
...
judge.on("newTask", () => {
    ...
    // When the user presses the mouse button on any scaling controller:
    scaling_controller.forEach(controller => {
        controller.on("mousedown", (e) => {
            interaction_type = 'scale';
            updateControllerIndicator(interaction_type, task.start.square, scaling_controller);
            initialScalePos = { x: e.clientX, y: e.clientY };
            // Prevent the event from bubbling up.
            e.stopPropagation();
        });
    });

    // While dragging, update the scale of the start square based on the distance dragged:
    svg.on("mousemove", (e) => {
        if (interaction_type === 'scale' && initialScalePos) {
            const bbox = manipulator.bbox();
            const center = { x: bbox.cx, y: bbox.cy };
            const dx = e.clientX - center.x;
            const dy = e.clientY - center.y;
            const dragDistance = Math.sqrt(dx * dx + dy * dy);
            const initDistance = Math.sqrt((initialScalePos.x - center.x) ** 2 + (initialScalePos.y - center.y) ** 2);

            dbg_curr_dist.innerHTML = dragDistance;
            dbg_orig_dist.innerHTML = `${initDistance}`;

            // Calculate a scale factor; adjust sensitivity as needed.
            const scaleFactor = dragDistance / initDistance;
            task.start.square.transform({ scale: scaleFactor, origin: 'center' });
        }
    });

    // When the mouse button is released, finish scaling:
    svg.on("mouseup", () => {
        if (interaction_type === 'scale') {
            interaction_type = 'none';
            updateControllerIndicator(interaction_type, task.start.square, scaling_controller);
            initialScalePos = null;
        }
    });
